...
function StatusChip({ status }) {
  if (status === 'in') {
    return (
      <span className="inline-flex items-center gap-1 text-[0.625rem] font-bold uppercase tracking-eyebrow-md font-mono text-emerald-signal">
  ...
  if (status === 'skipped') {
    return (
      <span className="inline-flex items-center gap-1 text-[0.625rem] font-bold uppercase tracking-eyebrow-md font-mono text-red-destructive/80">
        <X size={11} aria-hidden="true" /> Skipped
      </span>
    );
  }
  return (
    <span className="text-[0.625rem] font-bold uppercase tracking-eyebrow-md font-mono text-white/45">
      — Pending
    </span>
  );
}
  ...
  return (
    <div className="border border-white/8 bg-zinc-card/40 flex flex-col max-h-[70vh]">
      <div className="px-4 py-3 border-b border-white/8">
        <div className="flex items-center gap-2 text-[0.625rem] font-bold uppercase tracking-eyebrow-md font-mono">
        ...
        <p className="text-[0.625rem] font-mono text-white/35 mt-1">
      ...
      <div className="overflow-y-auto [scrollbar-width:thin] flex-1">
        {loading && !groups.length ? (
          <p className="text-center text-white/45 py-10 text-[0.6875rem] font-bold tracking-eyebrow-lg uppercase font-mono">
            Loading…
          </p>
        ) : groups.length === 0 ? (
          <p className="text-center text-white/50 py-10 text-[0.75rem] font-mono">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="text-[0.6875rem] font-bold tracking-eyebrow-md uppercase text-purple-bright font-mono truncate">
                      {g.person}
                    </span>
                    {isMe && (
                      <span className="text-[0.5625rem] font-bold uppercase tracking-eyebrow-md font-mono text-emerald-signal border border-emerald-signal/40 px-1 leading-tight">
                        you
                      </span>
                    )}
                  </div>
                  <ul className="space-y-1">
                    {(g.slots || []).map((s, si) => (
                      <li key={si} className="flex items-center justify-between gap-2">
                        <span className="text-[0.8125rem] text-white-body truncate">{s.name}</span>
