  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-[0.625rem] font-bold uppercase tracking-eyebrow-lg text-white/45 font-mono tabular-nums">
          {pendingCount} pending
        </span>
      </div>
      {intakeControls}
      {groups.length === 0 ? (
        <p className="text-center text-white/55 py-4 text-[0.75rem] font-mono">No pending calls.</p>
      ) : (
              ...
              <div className="flex items-center justify-between mb-1.5">
                ...
                {g.items.length > 1 && onSkipAll && (
                  <button type="button" onClick={() => onSkipAll(g.caller, g.items)}
                    className="text-[0.625rem] font-bold uppercase tracking-eyebrow-md font-mono text-white/45 hover:text-red-destructive">
                    Skip all
                  </button>
                )}
              </div>
                ...
                <div key={s.id} className="flex items-center justify-between gap-2 py-1">
                  ...
                  <div className="flex gap-1.5 shrink-0">
                    <button type="button" onClick={() => onSkip(g.caller, s)}
                      ...
                      className="inline-flex items-center gap-1 px-2 py-1 border border-white/10 text-white/55 hover:text-red-destructive hover:border-red-destructive/40 text-[0.625rem] font-bold uppercase tracking-eyebrow-md font-mono">
                      <X size={11} aria-hidden="true" /> Skip
                    </button>
                    <button type="button" onClick={() => onAdd(g.caller, s)}
                      className="inline-flex items-center gap-1 px-2 py-1 border border-emerald-signal/50 text-emerald-signal hover:bg-emerald-signal/10 text-[0.625rem] font-bold uppercase tracking-eyebrow-md font-mono">
